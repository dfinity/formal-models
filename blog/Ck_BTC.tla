...
CONSTANTS 
    ...
    \* This limits the total number of times that retrieve_btc can be called.
    \* This differs from update balance, where we just limit the number of concurrent calls.
    \* The reason is again that each retrieval creates a new Bitcoin transaction ID, so we
    \* need to limit the number of calls to keep the state finite.
...
not sensitive to the permutations of this set. We don't use symmetry reductions
when verify temporal properties, as these are tricky to get right with
symmetry reductions.
*)
Symmetry_Sets == { PRINCIPALS, SUBACCOUNTS, UPDATE_BALANCE_PROCESS_IDS, RETRIEVE_BTC_PROCESS_IDS }
...
\* Compute the new outputs needed to transfer the BTCs to serve a sequence of requests,
\* spending all of the given UTXOs
...
variables 
    ...
    btc = { [ id |-> << 0,0 >>, owner |-> OTHER_BTC_ADDRESSES, amount |-> BTC_SUPPLY - MINTER_INITIAL_SUPPLY] } 
        \union (IF MINTER_INITIAL_SUPPLY > 0
            THEN { [ id |-> << 0,1 >>, owner |-> MINTER_BTC_ADDRESS, amount |-> MINTER_INITIAL_SUPPLY] }
            ELSE {}
        );
    \**********************************************************************************************
    \* BTC Canister
    ...
    \* The buffers containing responses are also unordered
    btc_canister_to_minter = {};
    minter_to_ledger = <<>>;
    ledger_to_minter = {};
    \* We use a global counter a source of unique request IDs
    next_request_id = 1;
...
process (BTC = BTC_PROCESS_ID)
    variable nr_external_transfers = 0; 
{
BTC_Network_Loop:
    while(TRUE) {
        either {
            \* A transfer from a user-controlled address to a minter-controlled one 
            \* (up to MAX_EXTERNAL_BTC_TRANSFERS of such transfers)
            await(nr_external_transfers < MAX_EXTERNAL_BTC_TRANSFERS);
            with(user_utxos \in SUBSET Utxos_Owned_By(btc, {OTHER_BTC_ADDRESSES});
                    dest_address \in CK_BTC_ADDRESSES \union {MINTER_BTC_ADDRESS};
                    dest_amount \in 1..Sum_Utxos(user_utxos); 
                    transaction = [ 
                      consumed_utxos |-> user_utxos, 
                      outputs |-> New_Outputs(user_utxos, 
                        <<[address |-> dest_address, amount |-> dest_amount]>>, OTHER_BTC_ADDRESSES
                      ) 
                    ];
                    local_new_utxos = Utxos_Of(transaction)
                    ) {
                btc := (btc \ user_utxos) \union local_new_utxos;
                nr_external_transfers := nr_external_transfers + 1;
            }
        } or {
...
while(TRUE) {
  either {
    ...
    \* the real world by deferring any changes to the BTC network until the BTC 
    \* canister updates
    btc_canister := btc;
  } or {
      ...
      either {
        ...
        if(Is_Get_Utxos_Request(req)) {
          with(addr = Get_Utxos_Request_Address(req)) {
              respond_btc_canister_to_minter_utxos(
                Caller(req), 
...
Ledger_Loop:
    while(TRUE) {
        either {
            ...
            with(
                src_address \in DOMAIN balance \intersect CK_BTC_ADDRESSES; 
                dest_address \in 
                    CK_BTC_ADDRESSES 
                    \union 
                    { BTC_TO_WITHDRAWAL(x) :  x \in CK_BTC_ADDRESSES };
                amnt \in 1..balance[src_address];
                \* If dest_address isn't already in balances, set it to 0
                \* to simplify the computation below
                balance_with_default = balance @@ (dest_address :> 0)
            ) {
                balance := Remove_Balance_If_Zero(
                    src_address, 
                    [ balance_with_default EXCEPT 
                        ![src_address] = @ - amnt,
                        ![dest_address] = @ + amnt 
                    ]
                );
            ...
            with(req = Head(minter_to_ledger)) {
                ...
                either {
                    if(Is_Mint_Request(req)) {
                        balance := 
                            req.to :> (With_Default(balance, req.to, 0) + req.amount)
                            @@ balance;
                        ...
                        if(Is_Burn_Request(req)) {
                            if(req.address \in DOMAIN balance /\ balance[req.address] >= req.amount) {
                                balance := Remove_Balance_If_Zero(
                                    req.address, 
                                    [ balance EXCEPT ![req.address] = @ - req.amount]
                                );
...
process (Retrieve_BTC \in RETRIEVE_BTC_PROCESS_IDS)
    variable
        \* This variable models the parameter of the call. We don't model the choice of destination
        \* from the start (even though this is also a parameter), as this can be chosen later;
        \* we can ignore it if burning doesn't succeed so that we don't have to keep it in the state. 
        \* Also, we start with a default value for the amount, and choose the values only later, 
        \* once we start running the method, to reduce the state space.
        amount = 0;
{
Retrieve_BTC_Start:
...
Retrieve_BTC_Wait_Burn:
    ...
    with(
        response \in { r \in ledger_to_minter: Caller(r) = self }; 
        status = Status(response);
        \* Disable transfers to the minter BTC address when doing liveness checking
        destination \in CK_BTC_ADDRESSES \union { MINTER_BTC_ADDRESS, OTHER_BTC_ADDRESSES };
    ) {
        ledger_to_minter := ledger_to_minter \ {response};
        if(Is_Ok(status)) {
        ...
        new_transaction = <<>>;
{
    ...
    while(TRUE) {
        if (pending # <<>>) {
            ...
            \* Start_Submission:
            with(
                available_amount = Sum_Utxos(available_utxos);
                \* The requests that we can cover with the currently available UTXOs,
                \* without waiting for any change to arrive
                nr_servable_requests = Max(
                  {0} 
                  \union 
                  { i \in 1..Len(pending): Sum_Seq(Map(LAMBDA x: x.amount, SubSeq(pending, 1, i))) < available_amount }
                );
                requests_to_submit = SubSeq(pending, 1, nr_servable_requests);
                ...
                rest_pending = SubSeq(pending, nr_servable_requests + 1, Len(pending));
             ) {
                if (requested_amount > 0) {
                   with(
                       sset \in {s \in SUBSET available_utxos: 
                        ...
                        send_minter_to_btc_canister_submit(HEARTBEAT_PROCESS_ID, new_transaction);
                    }
                }
                else {
        ...
        Conclude_Submission:
        with(
            response \in { r \in btc_canister_to_minter: Caller(r) = HEARTBEAT_PROCESS_ID }; 
            status = Status(response); 
            change_index \in { i \in DOMAIN outputs : outputs[i].owner = MINTER_BTC_ADDRESS }
        ) {
            btc_canister_to_minter := btc_canister_to_minter \ {response};
            if(Is_Ok(status)) {
                submitted_transactions := 
                    ...
                    {[
                        ...
                        txid |-> Hash(new_transaction), 
                        used_utxos |-> spent, 
                        change_output |-> [vout |-> change_index, vamount |-> outputs[change_index].amount]
                    ]} ;
            } else {
                \* This puts the submission at the end of the queue; 
                \* this corresponds to the implementation, though for the model's purposes
        ...
        (* finalize_requests *)
        Finalize_Requests:  
        send_minter_to_btc_canister_get_utxos(HEARTBEAT_PROCESS_ID, MINTER_BTC_ADDRESS);
        ...
        with(
            ...
            if(Is_Ok(status)) {
                with(
                    utxos_from_result = Get_Utxos_Result(response); 
                    ...
                    new_finalized_utxos = [ 
                        principal \in {utxo.owner.owner : utxo \in confirmed_utxos} \intersect locks 
                        |-> 
                        { utxo \in confirmed_utxos : utxo.owner.owner = principal} \union With_Default(finalized_utxos, principal, {})
                    ];
                    trimmed_utxos_state_addresses_except_change = [
                        x \in {y \in DOMAIN utxos_states_addresses: utxos_states_addresses[y] \ confirmed_utxos /= {} } 
                        |-> 
                        utxos_states_addresses[x] \ confirmed_utxos
                    ];
                    trimmed_utxos_state_addresses_with_change = 
                        MINTER_BTC_ADDRESS 
                            :> With_Default(trimmed_utxos_state_addresses_except_change, MINTER_BTC_ADDRESS , {})\union new_utxos_finalize 
                        @@ trimmed_utxos_state_addresses_except_change; 
                ) {
                    (* 
...
*)
\* BEGIN TRANSLATION (chksum(pcal) = "610b5d94" /\ chksum(tla) = "74807767")
VARIABLES btc, btc_canister, utxos_states_addresses, available_utxos, 
          ...
          minter_to_ledger, ledger_to_minter, next_request_id, pc, 
          nr_external_transfers, caller_account, new_utxos, amount, submitted, 
          submitted_ids, spent, outputs, new_transaction

vars == << btc, btc_canister, utxos_states_addresses, available_utxos, 
           finalized_utxos, locks, pending, submitted_transactions, balance, 
           btc_canister_to_btc, minter_to_btc_canister, 
           btc_canister_to_minter, minter_to_ledger, ledger_to_minter, 
           next_request_id, pc, nr_external_transfers, caller_account, 
...
Init == (* Global variables *)
        ...
        /\ outputs = <<>>
        /\ new_transaction = <<>>
        /\ pc = [self \in ProcSet |-> CASE self = BTC_PROCESS_ID -> "BTC_Network_Loop"
...
BTC_Network_Loop == /\ pc[BTC_PROCESS_ID] = "BTC_Network_Loop"
                    /\ \/ /\ (nr_external_transfers < MAX_EXTERNAL_BTC_TRANSFERS)
                          /\ \E user_utxos \in SUBSET Utxos_Owned_By(btc, {OTHER_BTC_ADDRESSES}):
                       ...
                       \/ /\ \E submission \in { s \in btc_canister_to_btc: s.consumed_utxos \subseteq btc }:
                               LET local_new_utxos == Utxos_Of(submission) IN
                                 /\ btc_canister_to_btc' = btc_canister_to_btc \ {submission}
                                 /\ btc' = (local_new_utxos \union (btc \ submission.consumed_utxos))
                          /\ UNCHANGED nr_external_transfers
                    /\ pc' = [pc EXCEPT ![BTC_PROCESS_ID] = "BTC_Network_Loop"]
                    /\ UNCHANGED << btc_canister, utxos_states_addresses, 
                                    ...
                                    submitted, submitted_ids, spent, outputs, 
                                    new_transaction >>

BTC == BTC_Network_Loop
...
BTC_Canister_Loop == /\ pc[BTC_CANISTER_PROCESS_ID] = "BTC_Canister_Loop"
                           ...
                           /\ LET req == Head(minter_to_btc_canister) IN
                                ...
                                /\ \/ /\ IF Is_Get_Utxos_Request(req)
                                            ...
                                            ELSE /\ IF Is_Submission_Request(req)
                                                       ...
                                                       ELSE /\ Assert((FALSE), 
                                                                      "Failure of assertion at line 409, column 13.")
                                                            /\ UNCHANGED << btc_canister_to_btc, 
                                                                            btc_canister_to_minter >>
                     ...
                     /\ UNCHANGED << btc, utxos_states_addresses, 
                                     ...
                                     next_request_id, nr_external_transfers, 
                                     caller_account, new_utxos, amount, 
                                     submitted, submitted_ids, spent, outputs, 
                                     new_transaction >>
...
Ledger_Loop == /\ pc[LEDGER_PROCESS_ID] = "Ledger_Loop"
                          ...
                          /\ \/ /\ IF Is_Mint_Request(req)
                                      ...
                                      ELSE /\ IF Is_Burn_Request(req)
                                                 THEN /\ IF req.address \in DOMAIN balance /\ balance[req.address] >= req.amount
                                                            THEN /\ balance' =            Remove_Balance_If_Zero(
                                                                                   req.address,
                                                                                   [ balance EXCEPT ![req.address] = @ - req.amount]
                                                                               )
               ...
               /\ UNCHANGED << btc, btc_canister, utxos_states_addresses, 
                               ...
                               nr_external_transfers, caller_account, 
                               new_utxos, amount, submitted, submitted_ids, 
...
Update_Balance_Start(self) == /\ pc[self] = "Update_Balance_Start"
                              ...
                              /\ UNCHANGED << btc, btc_canister, 
                                              ...
                                              ledger_to_minter, 
                                              next_request_id, 
                                              nr_external_transfers, new_utxos, 
                                                   ...
                                                   THEN /\ LET utxos == Get_Utxos_Result(response) IN
                                                             LET nutxos ==          utxos \ (
                                                               ...
                                                               LET discovered_amount == Sum_Utxos(nutxos) IN
                                                                 ...
                                                                 /\ IF discovered_amount = 0
                                                                       THEN /\ locks' = locks \ {caller_account[self].owner}
                                                                       ...
                                                                       ELSE /\ minter_to_ledger' = Append(minter_to_ledger, Mint_Request(self, caller_account[self], discovered_amount))
                                         ...
                                         LET status == Status(response) IN
                                           ...
                                           /\ IF Is_Ok(status)
                                                 THEN /\ available_utxos' = (available_utxos \union new_utxos[self])
                                                      /\ utxos_states_addresses' = (caller_account[self] :> (
                                                                                        With_Default(utxos_states_addresses, caller_account[self], {})
                                                                                        \union
                                                                                        new_utxos[self]
                                                                                    )
...
Retrieve_BTC_Wait_Burn(self) == /\ pc[self] = "Retrieve_BTC_Wait_Burn"
                                /\ \E response \in { r \in ledger_to_minter: Caller(r) = self }:
                                     LET status == Status(response) IN
                                       \E destination \in CK_BTC_ADDRESSES \union { MINTER_BTC_ADDRESS, OTHER_BTC_ADDRESSES }:
                                         /\ ledger_to_minter' = ledger_to_minter \ {response}
                                         /\ IF Is_Ok(status)
...
Heartbeat_Start == /\ pc[HEARTBEAT_PROCESS_ID] = "Heartbeat_Start"
                   /\ IF pending # <<>>
                         THEN /\ LET available_amount == Sum_Utxos(available_utxos) IN
                                   LET nr_servable_requests ==                        Max(
                                                                 {0}
                                                                 \union
                                                                 { i \in 1..Len(pending): Sum_Seq(Map(LAMBDA x: x.amount, SubSeq(pending, 1, i))) < available_amount }
                                                               ) IN
                                     LET requests_to_submit == SubSeq(pending, 1, nr_servable_requests) IN
                                       LET requested_amount == Sum_Seq(Map(LAMBDA y: y.amount, requests_to_submit)) IN
                                         LET submitted_request_ids == Map(LAMBDA z: z.request_id, requests_to_submit) IN
                                           LET rest_pending == SubSeq(pending, nr_servable_requests + 1, Len(pending)) IN
                                             IF requested_amount > 0
                                                THEN /\ \E sset \in         {s \in SUBSET available_utxos:
                                                          ...
                                                          LET new_outputs == New_Outputs(sset, requests_to_submit, MINTER_BTC_ADDRESS) IN
                                                            ...
                                                            /\ new_transaction' = New_Submission(sset, new_outputs, 0)
                                                            /\ minter_to_btc_canister' =                       Append(minter_to_btc_canister,
                                                                                         Submission_Request(HEARTBEAT_PROCESS_ID, new_transaction'))
                                                     /\ pc' = [pc EXCEPT ![HEARTBEAT_PROCESS_ID] = "Conclude_Submission"]
                                                ELSE /\ submitted' = <<[request_id |-> 0, address |-> MINTER_BTC_ADDRESS, amount |-> 0]>>
                                                     /\ submitted_ids' = <<>>
                                                     /\ spent' = {}
                                                     /\ outputs' = <<>>
                                                     /\ new_transaction' = <<>>
                                                     /\ pc' = [pc EXCEPT ![HEARTBEAT_PROCESS_ID] = "Heartbeat_Start"]
                                                     /\ UNCHANGED << available_utxos, 
                                                                     pending, 
                                                                     minter_to_btc_canister >>
                         ELSE /\ pc' = [pc EXCEPT ![HEARTBEAT_PROCESS_ID] = "Finalize_Requests"]
                   ...
                   /\ UNCHANGED << btc, btc_canister, utxos_states_addresses, 
                                   finalized_utxos, locks, 
                                   submitted_transactions, balance, 
                                   btc_canister_to_btc, btc_canister_to_minter, 
                                   minter_to_ledger, ledger_to_minter, 
                                   next_request_id, nr_external_transfers, 
                                   caller_account, new_utxos, amount >>

Conclude_Submission == /\ pc[HEARTBEAT_PROCESS_ID] = "Conclude_Submission"
                       /\ \E response \in { r \in btc_canister_to_minter: Caller(r) = HEARTBEAT_PROCESS_ID }:
                            LET status == Status(response) IN
                              \E change_index \in { i \in DOMAIN outputs : outputs[i].owner = MINTER_BTC_ADDRESS }:
                                /\ btc_canister_to_minter' = btc_canister_to_minter \ {response}
                                /\ IF Is_Ok(status)
                                      THEN /\ submitted_transactions' = submitted_transactions
                                           ...
                                           /\ pc' = [pc EXCEPT ![HEARTBEAT_PROCESS_ID] = "Finalize_Requests"]
                                           /\ UNCHANGED << available_utxos, 
                                                           pending, submitted, 
                                                           submitted_ids, 
                                                           spent, outputs, 
                                                           new_transaction >>
                                      ELSE /\ available_utxos' = (available_utxos \union spent)
                                           /\ pending' = pending \o submitted
                                           /\ submitted' = <<[request_id |-> 0, address |-> MINTER_BTC_ADDRESS, amount |-> 0]>>
                                           /\ submitted_ids' = <<>>
                                           /\ spent' = {}
                       ...
                       /\ UNCHANGED << btc, btc_canister, 
                                       utxos_states_addresses, finalized_utxos, 
                                       locks, balance, btc_canister_to_btc, 
                                       minter_to_btc_canister, 
                                       minter_to_ledger, ledger_to_minter, 
                                       next_request_id, nr_external_transfers, 
                                       caller_account, new_utxos, amount >>

Finalize_Requests == /\ pc[HEARTBEAT_PROCESS_ID] = "Finalize_Requests"
                     /\ minter_to_btc_canister' =                       Append(minter_to_btc_canister,
                                                  Get_Utxos_Request(HEARTBEAT_PROCESS_ID, MINTER_BTC_ADDRESS))
                     /\ pc' = [pc EXCEPT ![HEARTBEAT_PROCESS_ID] = "Receive_Change_Utxos"]
                     /\ UNCHANGED << btc, btc_canister, utxos_states_addresses, 
                                     available_utxos, finalized_utxos, locks, 
                                     pending, submitted_transactions, balance, 
                                     btc_canister_to_btc, 
...
Receive_Change_Utxos == /\ pc[HEARTBEAT_PROCESS_ID] = "Receive_Change_Utxos"
                        /\ \E response \in { r \in btc_canister_to_minter: Caller(r) = HEARTBEAT_PROCESS_ID }:
                             LET status == Status(response) IN
                               /\ btc_canister_to_minter' = btc_canister_to_minter \ {response}
                               /\ IF Is_Ok(status)
                                                     ...
                                                     LET confirmed_transactions == {utxos[1] : utxos \in change_utxos_ids \intersect new_utxos_ids_finalize} IN
                                                       LET submitted_confirmed == {tx \in submitted_transactions: tx.txid \in confirmed_transactions} IN
                                                         LET confirmed_utxos == UNION{transaction.used_utxos : transaction \in submitted_confirmed} IN
                                                           LET new_finalized_utxos ==                       [
                                                                                          principal \in {utxo.owner.owner : utxo \in confirmed_utxos} \intersect locks
                                                                                          |->
                                                                                          { utxo \in confirmed_utxos : utxo.owner.owner = principal} \union With_Default(finalized_utxos, principal, {})
                                                                                      ] IN
                                                             LET trimmed_utxos_state_addresses_except_change ==                                               [
                                                                                                                    x \in {y \in DOMAIN utxos_states_addresses: utxos_states_addresses[y] \ confirmed_utxos /= {} }
                                                                                                                    |->
                                                                                                                    utxos_states_addresses[x] \ confirmed_utxos
                                                                                                                ] IN
                                                               LET trimmed_utxos_state_addresses_with_change == MINTER_BTC_ADDRESS
                                                                                                                    :> With_Default(trimmed_utxos_state_addresses_except_change, MINTER_BTC_ADDRESS , {})\union new_utxos_finalize
                                                                                                                @@ trimmed_utxos_state_addresses_except_change IN
                                                                 /\ utxos_states_addresses' = trimmed_utxos_state_addresses_with_change
                                                                 /\ submitted_transactions' = {tx \in submitted_transactions : tx.txid \notin confirmed_transactions}
                                                                 /\ finalized_utxos' = new_finalized_utxos @@ finalized_utxos
                                                                 /\ available_utxos' = (available_utxos \union new_utxos_finalize)
                                     ...
                                     ELSE /\ submitted' = <<[request_id |-> 0, address |-> MINTER_BTC_ADDRESS, amount |-> 0]>>
                                          ...
                                          /\ UNCHANGED << utxos_states_addresses, 
                                                          ...
                                                          finalized_utxos, 
                                                          submitted_transactions >>
                        /\ UNCHANGED << btc, btc_canister, locks, pending, 
                                        balance, btc_canister_to_btc, 
                                        minter_to_btc_canister, 
                                        minter_to_ledger, ledger_to_minter, 
                                        next_request_id, nr_external_transfers, 
...
Heartbeat == Heartbeat_Start \/ Conclude_Submission \/ Finalize_Requests
                \/ Receive_Change_Utxos
...
\* that our model can't do something we expect it to be able to do. Used only during
\* model development; don't use in the final analysis (as they should be violated).
...
Sanity_No_Simultaneous_Minter_And_User_Owned_BTC == 
    BTC_Balance_Of({MINTER_BTC_ADDRESS}) # 0 <=> BTC_Balance_Of({OTHER_BTC_ADDRESSES}) = 0
...
\* the BTC transfers are capped by MAX_EXTERNAL_BTC_TRANSFERS, and the number of retrievals is capped
...
\* minter's change address (or retrieve BTC to that address). We will also ignore such scenarios 
\* in the analysis, and disallow them in a separate constraint.
No_BTC_Left_Behind == 
...
Fairness_Condition ==
    ...
    /\ WF_vars(BTC_Network_Loop /\ btc_canister_to_btc # {} /\ btc_canister_to_btc' # btc_canister_to_btc)
    ...
    /\ \A pid \in HEARTBEAT_PROCESS_ID: WF_vars(Heartbeat)
 
 \* To achieve the equality between ckBTC supply and the sum of UTXOs under the minter's control,
\* we have to assume that the external users don't just transfer money to the ckBTC change address.
Prevent_External_Transfers_To_Change_Address ==
    ...
    \/ Utxos_Owned_By(btc, {OTHER_BTC_ADDRESSES}) \subseteq Utxos_Owned_By(btc', {OTHER_BTC_ADDRESSES})
